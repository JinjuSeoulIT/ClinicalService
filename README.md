...
| 기타 | MyBatis 스타터(매퍼 미사용 시 경고 로그만 발생할 수 있음), Lombok |
...
| 변수 | 설명 |
| --- | --- |
| `NEXT_PUBLIC_CLINICAL_API_BASE_URL` | 본 서비스 베이스 URL (코드 기본 예: `http://192.168.1.70:8090`, 미설정 시 해당 값 사용) |

`.env.local`에 실제 호스트·포트를 맞춥니다. 값이 비어 있으면 프론트가 기본 호스트에 붙거나 동작이 어긋날 수 있습니다.
...
- **백엔드**: `hospital-clinical` — 패키지 루트 `com.example.hospitalClinical`
